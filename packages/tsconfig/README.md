...
Beyond `strict`, the base enables the checks that catch the errors `strict`
alone does not:
...
`exactOptionalPropertyTypes` is left **off**. It is correct but produces
widespread churn in existing code; enable it per repository when ready.

`noEmit` is on — these configs are for type-checking. A package that emits
declares its own `outDir` and `noEmit: false`.
