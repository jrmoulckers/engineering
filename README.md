...
Private authority for ratified engineering principles and reusable implementations.
This repository is proprietary and unlicensed; no permission to use, copy, modify,
or distribute its contents is granted.
...
Product defines outcomes. Engineering implements mechanisms and evidence. Studio
expresses the UI. `jrmoulckers/.github` automates the work. The repository owner
alone ratifies engineering principles; agents may only propose changes.
...
an unmerged status change is only a proposal.

Three shared configuration packages are published from this repository:
`@jrmoulckers/eslint-config`, `@jrmoulckers/prettier-config`, and
`@jrmoulckers/tsconfig`. See [`docs/adopting.md`](docs/adopting.md).

## Consuming this repository

Cite principles by ID (`ENG-LOCAL-003`), never by copying their text — no
authority may restate another authority's normative text in its own source
tree. Resolve IDs to paths through `principles/index.json`. Install the shared
presets from GitHub Packages as devDependencies. Full instructions live in
...
Ratified principle `.md` files must not be edited: CI enforces an immutable
semantic hash over each one.
